...
int exit_error(char* msg, job* job_list){
    perror(msg);
...
int MultiPipeExecute(cmd_line *line){
    int fd[2],child1,child2;
    //fd[0]-> for using read end    fd[1]-> for using write end
...
int main(int argc, char *argv[]) {
    ...
    while(1){
        ...
        if(strcmp(s,"\n")!=0 || strcmp(s," \n")!=0 || strcmp(s,"    \n")!=0) {
            ...
            //check if the command is jobs-> if so- print as need, else- exec the command
            if (strcmp(s, "jobs\n") == 0 || strcmp(s, "jobs") == 0)
                print_jobs(&job_list);
            else {
                ...
                cpid = fork();
                if (cpid == -1) {
                    exit_error("cpid -fork() error",job_list);
                }
                if (cpid == 0) {
                    ...
                    if (line->next == 0)
                        ...
                        done = MultiPipeExecute(line);
                    }
                    if (done == -1) {
                        exit_error("illegal execute line",job_list);
                    }
                    exit(0);
                }
